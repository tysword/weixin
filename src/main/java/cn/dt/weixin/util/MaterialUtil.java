...
import cn.dt.weixin.message.material.BatchMaterial;

public class MaterialUtil {

	private static Logger log = LoggerFactory.getLogger(MaterialUtil.class);

	public static void sendMessage(BatchMaterial message, String token) throws Exception {
		...
		URI uri = new URIBuilder().setScheme("https").setHost(WeixinConstant.HOST_API)
				.setPath(WeixinConstant.PATH_METERIAL_BATCHGET).setParameters(params).build();
	...
	public static String convert2MessageJson(BatchMaterial message) {
		ObjectMapper mapper = new ObjectMapper();
		try {
